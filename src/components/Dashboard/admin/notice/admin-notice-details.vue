                ...
                <div v-else-if="notice" class="mx-auto">
                    ...
                    <div class="flex items-center justify-between mb-6">
                        ...
                        <div class="flex items-center gap-2">
                            <button
                                @click="editNotice"
                                class="inline-flex items-center gap-2 rounded-lg bg-amber-50 dark:bg-amber-500/10 px-3 py-1.5 text-xs font-semibold text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-500/20 transition">
                                <i class="fa-regular fa-pen-to-square"></i> Edit
                            </button>
                            <button
                                @click="deleteNotice"
                                class="inline-flex items-center gap-2 rounded-lg bg-rose-50 dark:bg-rose-500/10 px-3 py-1.5 text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-500/20 transition"
                            >
...
function editNotice() {
    router.push(`/admin/notice-edit/${route.params.id}`);
}
...
async function deleteNotice() {
    ...
    try {
        loading.value = true;
        await api.delete(`/notice/delete/${route.params.id}`);
        successMsg.value = 'Notice deleted successfully!';
        setTimeout(() => router.back(), 1000);
    } catch (err) {
        errorMsg.value = err.response?.data?.message || 'Failed to delete notice.';
    } finally {
        loading.value = false;
    }
}
